...
const { discordUser } = useDiscord()
  ...
  // Fetch NFT from blockchain
  // Fetch NFT from blockchain (read-only, no wallet popup)
const fetchUserNFT = async () => {
  if (!walletAddress) return null;
  
  try {
    // Use JsonRpcProvider for read-only operations (no wallet popup)
const provider = new ethers.providers.JsonRpcProvider(process.env.NEXT_PUBLIC_RPC_URL);
    const nftContract = new ethers.Contract(
      process.env.NEXT_PUBLIC_ORIGIN_NFT_ADDRESS,
      [
        "function addressToTokenId(address) view returns (uint256)",
        "function getNFTData(uint256) view returns (string,string,uint32,string,uint256)"
      ],
      provider  // Just provider, no signer needed
    );
    
    const tokenId = await nftContract.addressToTokenId(walletAddress);
    if (Number(tokenId) === 0) return null;
    
    const [imageURI, metadataURI] = await nftContract.getNFTData(tokenId);
    
    // If metadataURI exists, fetch the full metadata
    if (metadataURI) {
